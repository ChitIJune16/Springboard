...
SELECT COUNT(*) AS facilities_no_member_fee
FROM Facilities
WHERE membercost = 0;
...
FROM (
  SELECT
    ...
    b.slots * (
      CASE
        WHEN b.memid = 0
          THEN (SELECT guestcost  FROM Facilities WHERE facid = b.facid)
        ELSE (SELECT membercost FROM Facilities WHERE facid = b.facid)
      END
    ) AS cost
  FROM Bookings AS b
  WHERE DATE(b.starttime) = '2012-09-14'
) AS bc
JOIN Facilities AS f ON f.facid = bc.facid
JOIN Members   AS m ON m.memid = bc.memid
WHERE bc.cost > 30
